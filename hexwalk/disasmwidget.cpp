...
void DisasmWidget::capst(void)
{
    ...
    {
        message.asprintf("Invalid mode (e.g. x86 can't be Big Endian)");
    ...
    if (count > 0) {
        size_t j;
        for (j = 0; j < count; j++) {
            message.asprintf("0x%lX: %s %s", insn[j].address, insn[j].mnemonic,
                            insn[j].op_str);
            ui->lstDisasm->addItem(message);
    ...
    } else
    {
        ...
        message.asprintf("ERROR: Failed to disassemble given code!\n");
        ui->lstDisasm->addItem(message);
    }
    cs_close(&handle);
