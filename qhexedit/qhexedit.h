...
public:
    ...
    */
    ...
    /*! Calc cursor position from graphics position. GRAPHICS POSITION means the
    position of the cursor in pixel.
    \param point is the position in graphics coords
    */
    qint64 cursorPosition(QPoint point);
    
    /*! Get first visible byte address
     */
    qint64 firstByteAddress();

    /*! Ensure the selected data is visible.
    */
    void ensureVisible();
    ...
    /*! Searches in content for \param ba. Search starts at current cursor position.
    \param ba is the content to be find in QHexEdit content
    \param from is the start point of search
    ...
    \return This is the firts index of \param ba in content. In case, the \param ba
    is not found, 1 will be returned.
    */
    qint64 indexOf(const QByteArray &ba, qint64 from, bool isRegex, bool isCaseSensitive, bool invertMatch = false);

    /*! Returns if any changes where done on document since last save.
    */
    bool isModified();
    ...
    */
    ...
    /*! Gives back the length of the selected content. It's result can also become
    negative.
    */
...
signals:
    ...
    /*! The signal is emitted when user requests to add selected bytes as a byte pattern.
     *  \param start Starting byte position of the selection
     *  \param length Number of bytes selected
     *  \param data The actual byte data selected
     */
    void addBytePatternRequested(qint64 start, qint64 length, QByteArray data);
