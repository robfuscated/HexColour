...
void QHexEdit::showContextMenu(const QPoint &pos)
{
    ...
    if(!_editAreaIsAscii)
    {
        ...
        // Add separator before the new option
        if(getSelectionEnd() - getSelectionBegin() > 0)
        {
            contextMenu.addSeparator();
            
            // Add "Add Byte Pattern" option
            QAction *addPatternAction = contextMenu.addAction("Add Byte Pattern");
            connect(addPatternAction, &QAction::triggered, this, [this]() {
                // Get selection information
                qint64 selectionStart = getSelectionBegin();
                qint64 selectionLength = getSelectionEnd() - getSelectionBegin();
                
                // Get the selected data
                QByteArray selectedData = _chunks->data(selectionStart, selectionLength);
                
                // Emit the signal so the main window can handle it
                emit addBytePatternRequested(selectionStart, selectionLength, selectedData);
            });
        }
