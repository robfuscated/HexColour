...
void ConverterWidget::on_binTextEdit_textChanged(const QString &arg1)
{
    if(binValue != ui->binTextEdit->text())
    {
        ...
        if(check.length()){
            ...
            {
                if(check.at(i).unicode() != 0x30 && check.at(i).unicode() != 0x31)
                {
                    good = false;
                    break;
                }
...
void ConverterWidget::updateHex()
{
    ...
    if(check.length()){
        for(i=0;i<check.length();i++)
        {
            if(check.at(i).unicode() < 0x30 || (check.at(i).unicode() >0x39 && check.at(i).unicode() < 0x41) || (check.at(i).unicode() >0x46 && check.at(i).unicode() < 0x61) ||(check.at(i).unicode()> 0x66))
            {
                good = false;
                break;
            }
...
void ConverterWidget::updateDec()
{
    ...
    if(check.length())
    {
        for(i=0;i<check.length();i++)
        {
            if(check.at(i).unicode() < 0x30 || check.at(i).unicode() >0x39)
            {
                good = false;
                break;
            }
